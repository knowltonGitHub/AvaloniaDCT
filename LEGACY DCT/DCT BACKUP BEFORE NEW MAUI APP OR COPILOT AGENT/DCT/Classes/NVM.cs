...
namespace DCT.Classes
{
    public class NVM
    {
        ...
        public NVM()
        {
            TIV = new List<TagItemValue>() { };
        }

        public List<TagItemValue> TIV;
        ...
        public long IPAddressToLong(System.Net.IPAddress address)
        {
        ...
        //ce.AppEvent(new CustomEventArgs("NVM online", EventType.tagclick));
        //_tiv = new TagItemValue(Convert.ToInt32(this.UserID));

        public void DeleteInformationItemFromInMemory(string tempTag, string tempItem)
        {
            bool userIDIsValid = UserIDIsValid();

            TagItemValue removeThis = new TagItemValue();

            if (userIDIsValid)
            {
                foreach (TagItemValue tempTIV in TIV)
                {
                    if (tempTIV.Tag == tempTag)
                    {
                        if (tempTIV.Item == tempItem)
                        {
                            removeThis = tempTIV;
                        }
                    }
        ...
        public void DeleteExistingTagFromInMemory(string tempTag)
        {
            int tagcount = 0;
        ...
        public void FillTagItemValuesFromDatabase()
        {
            ...
            {
                m_dbConnection.Open();
                string sql = "select tag, item, thevalue, userid from tagitemvaltable " +
                    "where userid = " + UserID + " order by tag asc";
                ...
                while (query.Read())
                {
                    ...
                    tempTIV.Tag = query.GetString(0);
                    tempTIV.Item = query.GetString(1);
                    tempTIV.Value = query.GetString(2);
                    tempTIV.OwnerID = Convert.ToInt32(UserID);
        ...
        public ListBox GetAllTags()
        {
            ListBox lb = new ListBox();

            foreach (TagItemValue tempTIV in TIV)
            {
                if(!lb.Items.Contains(tempTIV.Tag))
                {
                    lb.Items.Add(tempTIV.Tag);
                }                
            }
        ...
        public ListBox GetAllItemsForSelectedTag(string tempTag)
        {
            ListBox lb = new ListBox();
            ...
            foreach (TagItemValue tempTIV in TIV)
            {
                if (tempTIV.Tag == tempTag)
                {
                    if (!lb.Items.Contains(tempTIV.Item))
                    {
                        lb.Items.Add(tempTIV.Item);
                    }
                }
            }
        ...
        public bool TagAlreadyExists(string tempTag)
        {
            bool tagWasFound = false;
        ...
        public bool ItemAlreadyExists(string tempTag, 
            string tempItem)
        {
        ...
        public void AddNewTagToInMemoryList(string tagCandidate)
        {
            if (!TagAlreadyExists(tagCandidate))
            {
                TagItemValue tempTIV = new TagItemValue();
                tempTIV.Tag = tagCandidate;
                tempTIV.Item = "";
                tempTIV.Value = "";
                tempTIV.OwnerID = Convert.ToInt32(UserID);
                TIV.Add(tempTIV);
        ...
        public string GetValueForInfoItem(string tempTag, string tempItem)
        {
            bool userIDIsValid = UserIDIsValid();
            string tempItemValue = EMPTYSTRING;     
        ...
        public void UpdateExistingInformationItemInMemoryList(string tempTag, string tempItem, string tempValue)
        {
            bool userIDIsValid = UserIDIsValid();
        ...
        //Only new information items should be saved
        public void SaveNewInformationItemToInMemoryList(string tempTag, string tempItem, string tempValue)
        {
            TagItemValue tempTIV = new TagItemValue();
            tempTIV.Tag = tempTag;
            tempTIV.Item = tempItem;
            tempTIV.Value = tempValue;
            tempTIV.OwnerID = Convert.ToInt32(UserID.ToString());
            TIV.Add(tempTIV);            
        }
        ...
        public void OverwriteTagItemsValueTableInDatabase()
        {
            ...
            if (userIDIsValid)
            {
                Helper.CreateCopyOfCurrentDB(TIV.Count(), "AUTO_BACKUP");
                m_dbConnection.Open();
